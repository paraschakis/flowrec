...
class FastBuffer(object):
    ...
    and predictions in a recommendation task context, so that we can keep updated
...
class FastComplexBuffer(object):
    ...
    predictions, one for each recommendation task. This structure is used to keep
    ...
    width: int
        The width from a complex entry, in other words how many recommendation
...
class ConfusionMatrix(object):
    ...
    ----------
    n_targets: int
        The number of targets from the single recommendation task associated
    ...
    def __init__(self, n_targets=None, dtype=np.int64):
        ...
        self.sample_count = 0
        self.dtype = dtype
        self.confusion_matrix = lil_matrix((n_targets, n_targets), dtype=self.dtype)
    ...
    def reshape(self, m, n):
        ...
        self.confusion_matrix = lil_matrix((m, n), dtype=self.dtype)
    ...
    def row(self, r):
        ...
        return self.confusion_matrix.getrow(r).toarray()
    ...
    def column(self, c):
        ...
        return self.confusion_matrix.getcol(c).toarray()
    ...
    def get_sum_main_diagonal(self):
        ...
        return sum(self.confusion_matrix.diagonal())
...
class MOLConfusionMatrix(object):
    ...
    recommendation tasks in a multi label environment. Thus, n_labels is
    always 2 (binary).

    The first dimension defines which recommendation task it keeps track of.
    ...
    [2, 1, 2] represents a miss recommendation in the recommendation task of
    ...
    ----------
    n_targets: int
        The number of recommendation tasks.
    ...
    Notes
    -----
    This structure starts with n_targets recommendation tasks. As the entries
    ...
    def value_at(self, target, i, j):
        ...
        ----------
        target: int
            An index from one of recommendation's tasks.
    ...
    def get_total_discordance(self):
        ...
        recommendation was detected. In other words it's the sum of all cells
...
class InstanceWindow(object):
    ...
    def add_all(self, X, Y):
        for i in range(len(X)):
            self.add_element(X[i:i + 1], np.array([Y[i:i + 1]]))

    def get_slice(self, values, feature_index=None):
        """Returns subset of the buffer with certain feature or target equal to value(s)

        Notes
        ----------
        Useful for retrieving session data

        Parameters
        ----------
        feature_index : int, optional (default=None)
            Index of feature column to search in. If None, search will be done in target.
        values : array or scalar
            Value(s) to search for. Must be of the same type as target column.

        Raises
        ------
        IndexError
            Instance window is empty or wrong column_index.

        Returns
        -------
        tuple
            A tuple of filtered attributes X and targets Y
        """

        X = self.get_attributes_matrix()
        Y = self.get_targets_matrix()
        try:
            if feature_index is None:
                ids = np.nonzero(np.isin(Y[:, 0], values))[0]
            else:
                ids = np.nonzero(np.isin(X[:, feature_index], values))[0]
        except IndexError:  # Instance window is empty or wrong column_index
            return np.empty((0, 0), dtype=float), np.empty((0, 0), dtype=float)
        return X[ids, :], Y[ids, :]

    # D: get the last record from the instance window for element in feature
    # D: (useful for retrieving the latest event data for the provided session)
    def get_last_entry(self, values, feature_index=None):
        """Returns the last record from the instance window for value(s) in feature or target

        Notes
        ----------
        Useful for retrieving the latest event for the provided session
        ...
        X, Y = self.get_slice(values, feature_index)
        try:
            return X[-1], Y[-1]
        except IndexError:  # Instance window is empty or wrong column_index
            return np.empty((0, 0), dtype=float), np.empty((0, 0), dtype=float)
